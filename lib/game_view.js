...
class GameView {
    ...
    renderPlayScreen(ctx) {
        ...
        for (let i = this.enemies.length - 1; i >= 0; i--) {
            this.game.trump.collided(this.enemies[0], this.window);
    ...
    addScore() {
        if (this.game.trump.pos[0] > (this.enemies[0].x - this.enemies[0].width) && 
        this.game.trump.pos[0] < (this.enemies[0].x - this.enemies[0].width) + 5) this.score++;
    }
